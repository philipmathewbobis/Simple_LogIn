<?php 
  ob_start(); // store output in the buffer before calling header() function
  session_start(); ?>
...
<?php
    if (!isset($_SESSION["user_id"]) && !isset($_SESSION["username"]) && !isset($_SESSION["name"]) ){
      ...
      if ($_SESSION["user_ip"] !== $_SERVER["HTTP_CLIENT_IP"] || $_SESSION["user_agent"] !== $_SERVER["HTTP_USER_AGENT"]){
        session_destroy(); // destroy session
        exit();
      }
    }else if (isset($_SERVER["HTTP_X_FORWARDED_FOR"])){
      if ($_SESSION["user_ip"] !== $_SERVER["HTTP_X_FORWARDED_FOR"] || $_SESSION["user_agent"] !== $_SERVER["HTTP_USER_AGENT"]){
        session_destroy(); // destroy session
        exit();
      }
    }
    include "../Flowbite Designs/header_inc.php";
    ob_end_flush(); // Sends the output from the buffer after calling the header() function
?>
      ...
      <div class="mb-4 grid gap-4 sm:grid-cols-2 sm:gap-8 lg:gap-16">
        <div class="space-y-4">
          <div class="flex space-x-4">
            <img class="h-16 w-16 rounded-lg" src="#" alt="<?php echo $_SESSION["name"]; ?>" />
            <div>
              ...
              <h2 class="flex items-center text-xl font-bold leading-none text-gray-900 dark:text-white sm:text-2xl"><?php echo $_SESSION["name"] ?></h2>
            </div>
          </div>
          <dl class="">
